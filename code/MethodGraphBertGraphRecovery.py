...
class MethodGraphBertGraphRecovery(BertPreTrainedModel):
    ...
    def train_model(self, max_epoch):
        ...
        for epoch in range(max_epoch):
            ...
            # TODO: check if we should send self.data['A'] to device before or after function .to_dense()
            loss_train = torch.sum((output - self.data['A'].to_dense().to(self.train_device)) ** 2)/(row_num*col_num)
